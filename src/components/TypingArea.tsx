                    ...
                    {visibleWords.map((originalWord, i) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={clsx(
                                    "relative px-2 py-1 rounded transition-colors duration-200",
                                    isCurrent ? "bg-white/10 text-white scale-110 z-10 shadow-glow" : "text-gray-500",
                                    isPast && "text-gray-700 opacity-50",
                                    // Level 2 Flicker classes applied conditionally
                                )}
                                style={{
                                    // Apply custom flicker animation if Level 2
                                    ...(level === 2 ? {
                            ...
                            >
                                ...
                                <span className="relative z-20">
                                    {isCurrent ? (
                                        <>
                                            ...
                                            {/* Space Warning - Outside the fading span so it persists */}
                                ...
                                {/* Progress Bar Overlay for Current Word */}
                                {isCurrent && (
                                    <motion.div
                                        className="absolute bottom-0 left-0 h-1 bg-cyan-400 z-30"
                                        initial={{ width: '100%' }}
                                        animate={{ width: `${wordTimer}%` }}
                                        transition={{ ease: 'linear', duration: 0.016 }} // Smooth frame update
                                    />
                                )}
                            </motion.div>
                        );
